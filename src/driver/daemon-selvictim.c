...
struct group_header *select_victim_greedy(struct dmsrc_super *super, int use_gc){
	...
	list_for_each_entry(group, &seg_allocator->group_sealed_queue, alloc_list){
		...
		//do nothing
			...
			// [TODO] 20180825 
			// Revive this part 
			// and replace the line below with this: if (group->phy_col_id != seg_allocator->req_write_col) {
...
//printk("[select_victim_for_gc]\n");
	...
	while(!list_empty(sealed_queue)){
		...
		if(super->param.victim_policy==VICTIM_GREEDY) {
			#if SWAN_Q2
			// [TODO] 20180825
			// Traverse not the particular group_alloc_queue of the col_Q but the whole group_alloc_queue 
			// And pick up the minimum victim group. 
			// group = select_victim_greedy(super, 1);
			// seg_allocator->gc_cold_write_col = group->phy_col_id;
