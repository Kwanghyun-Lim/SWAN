...
inline int can_get_free_segment(struct dmsrc_super *super, int cache_type, int gc){
	...
	#if SWAN_Q
	printk("[can_get_free_segment] @@@WARN@@@ cache_type %d need_size %d seg_alloccnt %u inactive_count %u inactive_page_count %u return 0;\n", 
		cache_type, need_size, 
		atomic_read(&seg_allocator->seg_alloc_count), 
		atomic_read(&segbuf_mgr->inactive_count), 
		atomic_read(&segbuf_mgr->inactive_page_count));	
		...
		int is_write)
{
	...
	if(is_write){
		...
		//printk("[preprocess_pending_bio] if(is_write)\n");
...
pending_process:
	printk("[preprocess_pending_bio] @@@@[WARN]@@@@ pended!! res:%d\n", res);
	return res;
...
static void set_default_param(struct dmsrc_super *super, struct dmsrc_param *param){
	...
	super->param.checker_interval = 10; 	/* 2sec -> 10sec */
