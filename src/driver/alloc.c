...
//		return;
	...
	if(super->param.hot_identification){
		...
		#if SWAN
		if( ( cache_type == SWAN_GC_CACHE_HOT || cache_type == SWAN_GC_CACHE_COLD )&& 
			(super->param.data_allocation==DATA_ALLOC_FLEX_VERT || 
			 super->param.data_allocation==DATA_ALLOC_FLEX_HORI)){
		...
		#if SWAN
		if((cache_type == SWAN_GC_CACHE_HOT ||cache_type == SWAN_GC_CACHE_COLD || cache_type==WCBUF||cache_type==GWBUF) && 
			(super->param.data_allocation==DATA_ALLOC_FLEX_VERT || 
			 super->param.data_allocation==DATA_ALLOC_FLEX_HORI)){
