...
#define SWAN			1
//#define SWAN_NUM_VICTIM_GROUP	15
//
//[PRINT]
#define SWAN_DBG 		1
#define SWAN_DBG_PRINT_ALLOC_Q	0
#define SWAN_ONLY_CHECKER_PROC 	0
//#define SWAN_GC_START_TIMING 	1  //[KH] THIS IS A COL NO.

#define SWAN_TRIM 		1

//[READ]
#define SWAN_READ_BLK_GC	0
#define SWAN_READ_BLK_W		0

//SWAN_Q
#define SWAN_Q			1	//rambuf division between GC and normal write
#define SWAN_Q2			1	//metadata queue (group_seg, seg) division between GC and normal write
// hot identification granularity
...
//[HOT/COLD]
#if 1 		
#define SWAN_MB_GRANULARITY	0
#define SWAN_SEG_GRANULARITY	0
#define SWAN_GRP_GRANULARITY	1		//should be always 1 whether hot/cold or not
#else
#define SWAN_MB_GRANULARITY	1
#define SWAN_SEG_GRANULARITY	0
#define SWAN_GRP_GRANULARITY	1
#endif

#define SWAN_HOTNESS_FLOAT	0
#define SWAN_HOTNESS_ULL	1

#define KH_DAEMON 		0
#define KH_INTERRUPT_PRINT 	1
...
#define NUM_SSD (super->param.ssd_row_num)
#define NUM_PHY_COL	(super->param.ssd_col_num)
#define NUM_PHY_SSD	NUM_SSD*NUM_PHY_COL
//#define NUM_FRONT_END_COL	2 // No more used
//#define NUM_BACK_END_COL	1 // No more used

#else

#define NUM_PHY_COL	1
...
#if SWAN
#define SEG_GROUP_FRIST_MB_IDX(g) SEG_START_IDX(g->group_id * SEGMENT_GROUP_SIZE)
#endif
...
#ifdef TRACE_GC
	atomic64_t normal_io_cnt[2]; 
	atomic64_t gc_io_cnt[2];
#endif
};
...
struct segment_allocator{
	...
	u32 gc_start_timing;
	u32 gc_read_col;
	u32 gc_hot_write_col;
	u32 gc_cold_write_col;
#endif

#if SWAN_Q2
	struct col_queue *col_queue_array;

        atomic_t group_alloc_count;
        atomic_t group_used_count;
        atomic_t group_sealed_count;
        atomic_t group_migrate_count;

        atomic_t seg_alloc_count;
        atomic_t seg_used_count;
        atomic_t seg_sealed_count;
        atomic_t seg_migrate_count;
...
struct dmsrc_super {
	...
	#if SWAN
	struct group_header *gc_cold_current_group;	//GC1
	struct group_header *gc_hot_current_group;	//GC2
	#endif

	#if SWAN_READ_BLK_GC
	atomic64_t num_read_inflight_in_col[16];
	#endif
	...
	#if SWAN_Q
	struct segbuf_manager segbuf_mgr1;	//GC1
	struct segbuf_manager segbuf_mgr2;	//GC2
	#endif
	...
	#if SWAN
	struct flush_manager flush_mgr1;	//GC1
	struct flush_manager flush_mgr2;	//GC2
	#endif
	...
	#if SWAN
	struct sync_manager sync_mgr1;		//GC1	partial seg flush
	struct sync_manager sync_mgr2;		//GC2	partial seg flush
	#endif
...
#if SWAN
int get_bdev_num_print(struct dmsrc_super *super, u32 idx);
#endif
