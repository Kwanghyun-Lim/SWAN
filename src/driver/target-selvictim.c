...
int change_seg_status(struct dmsrc_super *super, struct segment_header *seg, int seg_length, int force_seal){
	...
	if(force_seal || seg_length>=STRIPE_SZ){
		...
		if(test_bit(SEG_USED, &seg->flags)||test_bit(SEG_PARTIAL, &seg->flags)){
			...
			move_seg_used_to_sealed_queue(super, seg, col_Q);
			#else
...
inline int can_get_free_segment(struct dmsrc_super *super, int cache_type, int gc){
	...
	} else if (cache_type == SWAN_GC_CACHE_COLD) {
		segbuf_mgr = &super->segbuf_mgr1;
	} else {
		printk("[can_get_free_segment] cache_type error!!\n");
		BUG_ON(1);
	}

	if(atomic_read(&seg_allocator->seg_alloc_count)>=2&& 
	   atomic_read(&segbuf_mgr->inactive_count) && 
	   atomic_read(&segbuf_mgr->inactive_page_count)>=need_size)
	{
		//printk("[can_get_free_segment] return 1;\n");	
		return 1;
	}
	...
	//nothing
	#else
	if( atomic_read(&super->segbuf_mgr.inactive_count) && 
	...
	#if SWAN_Q
	printk("[can_get_free_segment] @@@WARN@@@ cache_type %d inactive_count %u inactive_page_count %u return 0;\n", cache_type, atomic_read(&segbuf_mgr->inactive_count), atomic_read(&segbuf_mgr->inactive_page_count));	
...
struct segment_header *_alloc_new_seg(struct dmsrc_super *super, int cache_type){
	...
	int req_write_col_free_grp_cnt = 0;
	...
	if (cache_type == SWAN_GC_CACHE_COLD) {
		group = super->gc_cold_current_group;
	}
	else if (cache_type == SWAN_GC_CACHE_HOT) {
		group = super->gc_hot_current_group;
	}
	else {
	...
	if(group==NULL || (group && atomic_read(&group->free_seg_count)==0)){

		#if SWAN_Q2
		spin_lock_irqsave(&seg_allocator->alloc_lock, flags);
		
		if (cache_type == SWAN_GC_CACHE_COLD) {
			phy_col_id = seg_allocator->gc_cold_write_col;
		} else if (cache_type == SWAN_GC_CACHE_HOT) {
			phy_col_id = seg_allocator->gc_hot_write_col;
		} else {
			phy_col_id = seg_allocator->req_write_col;
		}
		...
		#if SWAN_Q2
		if (cache_type == WCBUF) {
			req_write_col_free_grp_cnt = atomic_read(&col_Q->group_alloc_count);

			if ( req_write_col_free_grp_cnt <= 15 ) {
				atomic_set(&migrate_mgr->background_gc_on, 0);
				atomic_set(&migrate_mgr->migrate_triggered, 0);
				printk("[_alloc_new_seg] GC Finished ...\n");
			}

			if ( req_write_col_free_grp_cnt == 1 ) {
				...
				// [TODO] 20180825 
				// Choose the column which has the largest free group segments number.
				// for (int i = 0; i = NUM_PHY_COL; i++) {
				// 	int max_free_col = 0;
				// 	
				// 	if (atomic_read(&seg_allocator->col_queue_array[i]->group_alloc_count > atomic_read(&seg_allocator->col_queue_array[max_free_col]) {
				// 		max_free_col = i;
				// 		}
				// seg_allocator->req_write_col = max_free_col;
				
				///////////////// 20180825
				// Replace those three lines below with the lines above.
		...
		group = remove_alloc_group_queue(super, col_Q, cache_type);
		...
		if(atomic_read(&super->seg_allocator.group_alloc_count)<=1){
			printk("[_alloc_new_seg] WARN: Alloc seg free = %d \n", (int)get_alloc_count(super));
			printk("[_alloc_new_seg] WARN: Alloc group free = %d \n", (int)get_group_alloc_count(super));
		...
		if(group==NULL){
		#endif
			// [TODO] check if all the other columns but group->phy_col_id have no free group_seg 
			// 
			printk("[_alloc_new_seg] cache_type %u Alloc cur group free = %d \n", cache_type, (int)atomic_read(&super->seg_allocator.group_alloc_count));
			printk("[_alloc_new_seg] cache_type %u Alloc seg free = %d \n", cache_type, (int)get_alloc_count(super));
			printk("[_alloc_new_seg] cache_type %u Alloc group free = %d \n", cache_type, (int)get_group_alloc_count(super));
			#if SWAN
				schedule_timeout_interruptible(usecs_to_jiffies(50000));
				printk("[_alloc_new_seg] @@@ interrupt @@@\n");
			#else
				BUG_ON(1);
			#endif
		}
		...
		#if SWAN
		if (cache_type == SWAN_GC_CACHE_COLD) {
			super->gc_cold_current_group = group;
		}
		else if (cache_type == SWAN_GC_CACHE_HOT) {
			super->gc_hot_current_group = group;
		}
		else {
...
//	printk(" alloc group = %d seg = %d \n", (int)group->group_id, (int)group->current_seg_id);
	...
	col_Q = &seg_allocator->col_queue_array[phy_col_id];
	remove_alloc_queue(super, new_seg, col_Q);
	#else
...
u64 alloc_new_segment(struct dmsrc_super *super, int cache_type, bool use_migrate)
{
	...
	#if SWAN_Q2
	phy_col_id = new_seg->group->phy_col_id;
	col_Q = &seg_allocator->col_queue_array[phy_col_id];
	insert_seg_to_used_queue(super, new_seg, col_Q);
	#else
...
void wait_rambuf_event(struct dmsrc_super *super, int cache_type){
	...
	while( !atomic_read(&super->segbuf_mgr.inactive_count) || 
		...
		printk("[wait_rambuf_event] @@@ interrupt @@@\n");
		...
		u32 *total_count)
{
	...
	struct group_header *grp = seg->group;

	if (cache_type == WCBUF) {
		grp->W_freq++;
		grp->T_prev = jiffies;
		//printk("[alloc_mb_data] new_mb->W_freq %llu new_mb->T_prev %lu \n", new_mb->W_freq, new_mb->T_prev);
	}
...
#if SWAN
	u32 idx_phy_ssd;
	idx_phy_ssd = idx / (super->cache_stat.num_segments * STRIPE_SZ);
	dev_no = dev_no + NUM_SSD * idx_phy_ssd;
#endif
...
inline int get_bdev_num_print(struct dmsrc_super *super, u32 idx){
	int dev_no;
	dev_no = idx / CHUNK_SZ % NUM_SSD;
...
#if SWAN
	seg_id = seg_id % super->cache_stat.num_segments;
#endif
...
void make_flush_invoke_job(struct dmsrc_super *super, struct segment_header *seg, 
	...
	if (cache_type == WCBUF) {
		 flush_mgr = &super->flush_mgr;
	} else if (cache_type == SWAN_GC_CACHE_HOT) {
		 flush_mgr = &super->flush_mgr1;
	} else { // cache_type == SWAN_GC_CACHE_COLD
		 flush_mgr = &super->flush_mgr2;
	}
...
static int process_read_miss_request(struct dmsrc_super *super, struct bio *bio, unsigned long f){
	...
	#if SWAN
		bio_endio(bio,0);
		//bio_remap(bio, super->dev_info.cache_dev[0], bio->bi_sector%(super->cache_stat.num_blocks_per_ssd*8));
		//printk("[process_read_miss_request] SWAN_WARN : bio_endio(bio,0)!\n");
	#else
...
static int process_read_hit_request(struct dmsrc_super *super, struct bio *bio,
	...
	struct migration_manager *migrate_mgr = &super->migrate_mgr;
        ...
        atomic64_inc(&super->num_read_inflight_in_col[phy_col_id]);
...
int try_wait_free_seg(struct dmsrc_super *super, int cache_type){
	...
	if(should_need_refresh_seg(super, cache_type)){
		...
		//if (get_grp_alloc_count_of_gc_cold_write_col(super) < SWAN_NUM_VICTIM_GROUP || get_grp_alloc_count_of_gc_hot_write_col(super) < SWAN_NUM_VICTIM_GROUP) {
#else
		if(atomic_read(&super->seg_allocator.seg_alloc_count)<MIGRATE_LOWWATER || 
			atomic_read(&super->seg_allocator.group_alloc_count)<=1){
			if(!atomic_read(&super->migrate_mgr.migrate_triggered)){
				...
				wake_up_process(super->migrate_mgr.daemon);
				#if SWAN_DBG
					printk("[try_wait_free_seg] migration daemon is invoked ... \n");
				#endif
			}
			#if SWAN
			//return 1; //retrun 1 : seperatly conduct GC and write 
				  //no return : simultaneously conduct GC and write (concept of SWAN) 
			#else
			return 1;
			#endif
		}
		...
		int is_write)
{
	...
	if(mb){ //if Updated
		...
		{
			printk("[preprocess_pending_bio] @@@@[WARN]@@@@ #1\n");
			res = RES_PARTIAL;
			goto pending_process;
		}

		if(unlikely(!test_bit(MB_SEAL, &mb->mb_flags))){
			//printk("[preprocess_pending_bio] @@@@[WARN]@@@@ #2\n");
			if(!is_write){
			printk("[preprocess_pending_bio] @@@@[WARN]@@@@ #2_1\n");
				res = RES_SEAL;
				goto pending_process;
			}//else{
			//	printk(" mb write hit .... \n");
	...
	if(is_write){
		...
		//printk("[preprocess_pending_bio] #3\n");
...
pending_process:
	printk("[preprocess_pending_bio] @@@@[WARN]@@@@ pended!!\n");
	return res;
...
RETRY:
			...
			if(try_wait_free_seg(super, cache_type)){
				...
				while(1){
					...
					printk("[clean_seg_write] @@@ interrrupt @@@\n");
...
void pending_worker(struct work_struct *work){
	...
	while (1) {
		...
		if(!bio) {
			//printk("[pending_worker] @@@WARN@@@ no bio!!\n");
			break;
		}
		...
		if(reason<0){
			printk("[pending_worker] @@@ reason %d @@@\n", reason);
...
//#if SWAN_MB_GRANULARITY || SWAN_SEG_GRANULARITY || SWAN_GRP_GRANULARITY
#if SWAN_MB_GRANULARITY 
	...
	if(!bio_data_dir(bio)){
		//printk("[dmsrc_map] if(!bio_data_dir(bio)) \n");
		struct metablock *mb = NULL;
		struct segment_header *seg = NULL;
		unsigned long f;
		int ret;
		...
		mb = ht_lookup(super, key);
		if (!mb) { // no ssd hit 
			//printk("[dmsrc_map] if(!(mb)) \n");
			if(super->param.enable_read_cache){
				//printk("[dmsrc_map] if(super->param.enable_read_cache == 1) \n");
				struct cache_manager *clean_cache = super->clean_dram_cache_manager;
				unsigned long lru_flags; 
				struct lru_node *ln = NULL;
				...
				if(ln && atomic_read(&ln->sealed) && !atomic_read(&ln->locked)){
					...
					/*
					#if SWAN
						printk("[dmsrc_map] bio_endio(bio,0); \n");
						spin_unlock_irqrestore(&clean_cache->lock, lru_flags);
						UNLOCK(super, f);
						bio_endio(bio,0);
						return DM_MAPIO_SUBMITTED;
			...
			//printk("[dmsrc_map] goto pending manager #1 \n");
			...
			//printk("[dmsrc_map] goto pending manager #2 \n");
			// goto pending manager 
		}
		UNLOCK(super, f);
	}
...
void do_grow(struct dmsrc_super *super){
	...
	for(i = 0;i < NBUF;i++){ //not used in SWAN, so KH temperarily removes it.
...
static int dmsrc_end_io(struct dm_target *ti, struct bio *bio, int error)
{
	...
	struct group_header *grp;
        ...
        grp = seg->group;
        phy_col_id = grp->phy_col_id;
        atomic64_dec(&super->num_read_inflight_in_col[phy_col_id]);
...
static int pending_mgr_init(struct dmsrc_super *super){
	...
	#if 0
		#if SWAN_GC_1
			pending_mgr->wb_job_pool = mempool_create_kmalloc_pool(STRIPE_SZ*2,
									    sizeof(struct wb_job));
		#endif

		#if SWAN_GC_2
			pending_mgr->wb_job_pool = mempool_create_kmalloc_pool(STRIPE_SZ*3,
									    sizeof(struct wb_job));
		#endif
...
void seg_allocator_init(struct dmsrc_super *super){
	...
	printk("[seg_allocator_init] #1\n");

#if SWAN_Q2
	int i;
	spin_lock_init(&seg_allocator->alloc_lock);

	seg_allocator->col_queue_array = (struct col_queue *)vmalloc(sizeof(struct col_queue) * NUM_PHY_COL);
	
	if (seg_allocator->col_queue_array == NULL) {
		printk("[seg_allocator_init] vmalloc error %s %d\n", __FUNCTION__, __LINE__);
		BUG_ON(1);
	}
	
	printk("[seg_allocator_init] #2\n");

	for ( i = 0; i < NUM_PHY_COL; i++) {
		printk("[seg_allocator_init] #3 i %d\n", i);
		struct col_queue *col_Q = &seg_allocator->col_queue_array[i];

		spin_lock_init(&col_Q->col_queue_lock);
	
		INIT_LIST_HEAD(&col_Q->group_alloc_queue);
		INIT_LIST_HEAD(&col_Q->group_migrate_queue);
		INIT_LIST_HEAD(&col_Q->group_used_queue);
		INIT_LIST_HEAD(&col_Q->group_sealed_queue);

		atomic_set(&col_Q->group_alloc_count, 0); 
		atomic_set(&col_Q->group_migrate_count, 0); 
		col_Q->group_used_count = 0;
		col_Q->group_sealed_count = 0;

		INIT_LIST_HEAD(&col_Q->seg_alloc_queue);
		INIT_LIST_HEAD(&col_Q->seg_migrate_queue);
		INIT_LIST_HEAD(&col_Q->seg_used_queue);
		INIT_LIST_HEAD(&col_Q->seg_sealed_queue);

		atomic_set(&col_Q->seg_alloc_count, 0); 
		atomic_set(&col_Q->seg_migrate_count, 0); 
		col_Q->seg_used_count = 0;
		col_Q->seg_sealed_count = 0;
	}
	
	printk("[seg_allocator_init] #4\n");

	atomic_set(&seg_allocator->group_used_count, 0);
	atomic_set(&seg_allocator->group_sealed_count, 0);

	atomic_set(&seg_allocator->seg_used_count, 0);
	atomic_set(&seg_allocator->seg_sealed_count, 0);
	
	printk("[seg_allocator_init] #5\n");
#else
	...
	seg_allocator->req_write_col = 0;
	seg_allocator->gc_start_timing = NUM_PHY_COL - 1; 
	//seg_allocator->gc_start_timing = 1; 
	//seg_allocator->gc_cold_write_col = 3;
	seg_allocator->gc_cold_write_col = 1;
	seg_allocator->gc_hot_write_col = 3;
	

	//seg_allocator->gc_read_col = 0;
	//seg_allocator->gc_hot_write_col = 0;
#endif
...
int flush_mgr_init(struct dmsrc_super *super){
	...
	int r = 0;
	#if SWAN
	struct flush_manager *flush_mgr1 = &super->flush_mgr1;
	struct flush_manager *flush_mgr2 = &super->flush_mgr2;
	#endif
	...
	spin_lock_init(&flush_mgr1->lock);
	atomic_set(&flush_mgr1->invoke_count, 0);
	atomic_set(&flush_mgr1->io_count, 0);
	atomic64_set(&flush_mgr1->global_seg_sequence, 0);
	INIT_LIST_HEAD(&flush_mgr1->queue);

	r = create_daemon(super, &flush_mgr1->daemon, flush_meta_proc1, "flush_meta_daemon1"); 
	if(r)
		goto bad_init;

	flush_mgr1->invoke_pool = mempool_create_kmalloc_pool(100,
							    sizeof(struct flush_invoke_job));
	if (!flush_mgr1->invoke_pool) {
		r = -ENOMEM;
		WBERR("couldn't alloc flush job pool");
		goto bad_init;
	}

	flush_mgr1->initialized = 1;

	spin_lock_init(&flush_mgr2->lock);
	atomic_set(&flush_mgr2->invoke_count, 0);
	atomic_set(&flush_mgr2->io_count, 0);
	atomic64_set(&flush_mgr2->global_seg_sequence, 0);
	INIT_LIST_HEAD(&flush_mgr2->queue);

	r = create_daemon(super, &flush_mgr2->daemon, flush_meta_proc2, "flush_meta_daemon2"); 
	if(r)
		goto bad_init;

	flush_mgr2->invoke_pool = mempool_create_kmalloc_pool(100,
							    sizeof(struct flush_invoke_job));
	if (!flush_mgr2->invoke_pool) {
		r = -ENOMEM;
		WBERR("couldn't alloc flush job pool");
		goto bad_init;
	}

	flush_mgr2->initialized = 1;
	#endif
	printk("[flush_mgr_init]\n");
...
void flush_mgr_deinit(struct dmsrc_super *super){
	...
	#if SWAN
	while(atomic_read(&super->flush_mgr.invoke_count) ||
		atomic_read(&super->flush_mgr1.invoke_count) ||
		atomic_read(&super->flush_mgr2.invoke_count) ){

		schedule_timeout_interruptible(msecs_to_jiffies(1000));
	}

	while(atomic_read(&super->flush_mgr.io_count) ||
		atomic_read(&super->flush_mgr1.io_count) ||
		atomic_read(&super->flush_mgr2.io_count)){

		schedule_timeout_interruptible(msecs_to_jiffies(1000));
	}

	kthread_stop(super->flush_mgr.daemon);
	kthread_stop(super->flush_mgr1.daemon);
	kthread_stop(super->flush_mgr2.daemon);
	mempool_destroy(super->flush_mgr.invoke_pool);
	mempool_destroy(super->flush_mgr1.invoke_pool);
	mempool_destroy(super->flush_mgr2.invoke_pool);
...
int migrate_mgr_init(struct dmsrc_super *super){
	...
	#if KH_DAEMON
	create_daemon(super, &migrate_mgr->daemon2, migrate_proc2, "migrate_daemon2"); 
	#endif
...
enum hrtimer_restart sync_timer_callback1( struct hrtimer *timer )
{
	//printk("[sync_timer_callback1]\n");
...
#else
	 schedule_work(&super->sync_mgr1.work);
#endif 
	
	return HRTIMER_NORESTART;
}

enum hrtimer_restart sync_timer_callback2( struct hrtimer *timer )
{
	//printk("[sync_timer_callback2]\n");
	struct sync_manager *sync_mgr = container_of(timer, struct sync_manager, hr_timer);
	struct dmsrc_super *super = sync_mgr->super;
...
#else
	 schedule_work(&super->sync_mgr2.work);
#endif 
...
int sync_mgr_init(struct dmsrc_super *super){
	...
	#if SWAN
	struct sync_manager *sync_mgr1 = &super->sync_mgr1;
	struct sync_manager *sync_mgr2 = &super->sync_mgr2;
	#endif
	...
	#if SWAN
	spin_lock_init(&sync_mgr1->lock);
	sync_mgr1->super = super;
	super->param.sync_interval = DEFAULT_ARRIVAL_US;

	hrtimer_init( &sync_mgr1->hr_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL );
	sync_mgr1->hr_timer.function = &sync_timer_callback1;


	//setup_timer(&sync_mgr->timer,
	//	    schedule_sync_proc, (unsigned long) super);
	INIT_WORK(&sync_mgr1->work, sync_proc1);
	sync_mgr1->initialized = 1;


	spin_lock_init(&sync_mgr2->lock);
	sync_mgr2->super = super;
	super->param.sync_interval = DEFAULT_ARRIVAL_US;

	hrtimer_init( &sync_mgr2->hr_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL );
	sync_mgr2->hr_timer.function = &sync_timer_callback2;


	//setup_timer(&sync_mgr->timer,
	//	    schedule_sync_proc, (unsigned long) super);
	INIT_WORK(&sync_mgr2->work, sync_proc2);
	sync_mgr2->initialized = 1;
...
void sync_mgr_deinit(struct dmsrc_super *super){
	...
	#if SWAN
	if(!sync_mgr1->initialized)
		return;
	if(!sync_mgr2->initialized)
		return;
	#endif
	...
	#if SWAN
	flush_work(&sync_mgr1->work);
	cancel_work_sync(&sync_mgr1->work);
	flush_work(&sync_mgr2->work);
	cancel_work_sync(&sync_mgr2->work);
	#endif
}
...
void update_sync_deadline(struct dmsrc_super *super)
{
	...
	//printk("[update_sync_deadline] update sync ... timer ... \n");
	sync_mgr->period = ktime_set(0, usec*1000);
	hrtimer_start( &sync_mgr->hr_timer, sync_mgr->period, HRTIMER_MODE_REL );
	#if SWAN
	sync_mgr1->period = ktime_set(0, usec*1000);
	sync_mgr2->period = ktime_set(0, usec*1000);
	hrtimer_start( &sync_mgr1->hr_timer, sync_mgr1->period, HRTIMER_MODE_REL );
	hrtimer_start( &sync_mgr2->hr_timer, sync_mgr2->period, HRTIMER_MODE_REL );
	#endif
}
...
int hot_filter_check(struct dmsrc_super *super, unsigned int pageno){
	...
	//printk("[hot_filter_check] pageno = %u \n",pageno);
	...
	#if 1
	spin_lock_irqsave(&seg_allocator->alloc_lock, flags2);
	if(src_hot_count > (seg_allocator->gc_cold_write_col + 1)) {
		spin_unlock_irqrestore(&seg_allocator->alloc_lock, flags2);
		return 1;
	} else {
		spin_unlock_irqrestore(&seg_allocator->alloc_lock, flags2);
		return 0;
	...
	#else
	spin_lock_irqsave(&seg_allocator->alloc_lock, flags2);
	printk("[hot_filter_check] #1 NUM_BLOCKS: %d\n", NUM_BLOCKS);
	
	for (i = seg_allocator->gc_cold_write_col * NUM_BLOCKS; i < seg_allocator->gc_cold_write_col * NUM_BLOCKS + NUM_BLOCKS; i++) {
		printk("[hot_filter_check] #2 i %u\n", i);
		/*
		hot_count = 0;
		struct metablock *mb = mb_at(super, i);
		pageno = mb->sector;	
		...
		if (src_hot_count < hot_count) {	// Calculate the grade of src_hot_count in gc_write_col
			cnt++;
		}
		*/
	}	
	
	//printk("[hot_filter_check] #2\n");
	
	for (i = seg_allocator->gc_hot_write_col * NUM_BLOCKS; i < seg_allocator->gc_hot_write_col * NUM_BLOCKS + NUM_BLOCKS; i++) {
		printk("[hot_filter_check] #3 i %u\n", i);
		/*
		hot_count = 0;
		struct metablock *mb = mb_at(super, i);
		pageno = mb->sector;
	...
	printk("[hot_filter_check] #4\n");
	spin_unlock_irqrestore(&seg_allocator->alloc_lock, flags2);
	
	if (cnt > NUM_BLOCKS) {	//cold
		return 0;	
	} else {		//hot
		return 1;
	}
	...
	#else
	if(src_hot_count>=hot_filter->hot_threshold) {	//hot
		return 1;
	} else {					//cold
		return 0;
	}
...
void hot_filter_update(struct dmsrc_super *super, unsigned int pageno){
	...
	//printk("[hot_filter_update] pageno = %u \n",pageno);
	offset[0] = crc32(17, (unsigned char *)&pageno, sizeof(unsigned int)) % hot_filter->num_bits_per_table; 
	offset[1] = jhash_1word(pageno, 0xbeef) % hot_filter->num_bits_per_table;
	offset[2] = hash_32(pageno, 32) % hot_filter->num_bits_per_table;
...
int hot_filter_init(struct dmsrc_super *super){
	...
	#if SWAN
	hot_filter->bitmap_table_num = 4 * NUM_PHY_COL;	//#define MAX_BITMAP_TABLE 	4 * NUM_PHY_COL
	if (hot_filter->bitmap_table_num > MAX_BITMAP_TABLE) {
		printk("[hot_filter_init] @@@@@@@@@@@@@@NO!!!!!!!!!!@@@@@@@@@@@@@@@@@\n");
	}
	#else
	hot_filter->bitmap_table_num = 4;	//#define MAX_BITMAP_TABLE	4
	#endif
	...
	printk("[hot_filter_init] num bits per table = %d \n", hot_filter->num_bits_per_table);
...
int __must_check resume_managers(struct dmsrc_super *super)
{
	...
	int i;
        for (i = 0; i < 16; i++) {
                printk("[resume_managers] atomic64_set(&super->num_read_inflight_in_col[%d], 0);\n", i);
                atomic64_set(&super->num_read_inflight_in_col[i], 0);
        }
